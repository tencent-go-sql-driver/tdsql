...
func testDialError(t *testing.T, dialErr error, expectErr error) {
	...
	db, err := sql.Open("tdsql", fmt.Sprintf("%s:%s@mydial(%s)/%s?timeout=30s", user, pass, addr, dbname))
	if err != nil {
		t.Fatalf("error connecting: %s", err.Error())
	}
	defer db.Close()
...
func TestConnectorObeysDialTimeouts(t *testing.T) {
	...
	db, err := sql.Open("tdsql", fmt.Sprintf("%s:%s@dialctxtest(%s)/%s?timeout=30s", user, pass, addr, dbname))
	if err != nil {
		t.Fatalf("error connecting: %s", err.Error())
	}
	defer db.Close()

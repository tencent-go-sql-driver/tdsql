...
## Usage
_Go TDSQL Driver_ is an implementation of Go's `database/sql/driver` interface. You only need to import the driver and can use the full [`database/sql`](https://golang.org/pkg/database/sql/) API then.

Use `tdsql` as `driverName` and a valid [DSN](#dsn-data-source-name)  as `dataSourceName`:
